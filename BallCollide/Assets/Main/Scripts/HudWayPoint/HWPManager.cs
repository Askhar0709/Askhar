...
public class HWPManager : MonoBehaviour
{
    ...
    void OnScreen(int i)
    {
        ...
        if (HWPUtility.isOnScreen(HWPUtility.ScreenPosition(Huds[i].m_Target), Huds[i].m_Target) &&
            A.IsPlaying && // Тоглох
            Huds[i].character == null ? true : Huds[i].character.data.isLive && // амьд
    ...
    void OffScreen(int i)
    {
            ...
            (A.FollowType != FollowType.Follow && A.FollowType != FollowType.Hud)) // заагчтай
        {
            ...
            if (!HWPUtility.isOnScreen(HWPUtility.ScreenPosition(Huds[i].m_Target), Huds[i].m_Target))
            {
                ...
                //Arrow
                Vector2 rotVec = new Vector2(A.Cos(mRot), A.Sin(mRot));
    ...
    private void Palpating(HWPInfo hud)
    {
        ...
        if (hud.tip == false)
        {
            hud.color.a += Time.deltaTime * 0.5f;
        }
        else
        {
            hud.color.a -= Time.deltaTime * 0.5f;
